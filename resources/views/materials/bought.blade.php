            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="storeBoughtMaterialLabel">{{ __('app.add.bought') }}</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <form class="d-flex text-center flex-column" action="/bought-materials" method="post">
                            @csrf
                            <div class="m-3">
                                <label for="bought_on" class="form-label">{{ __('app.fields.date') }}*</label>
                                <input type="date" class="form-control" id="bought_on" name="bought_on"
                                    value="{{ old('bought_on') }}">
                            </div>
                            <div class="m-3">
                                <label for="partner_id" class="form-label">{{ __('app.fields.bought_from') }}*</label>
                            ...
                            <div class="m-3">
                                <label for="material_id"
                                    class="form-label">{{ __('app.fields.bought_material') }}*</label>
                            ...
                            <div class="m-3">
                                <label for="quantity"
                                    class="form-label">{{ __('app.fields.bought_quantity') }}*</label>
            ...
            <table class="table table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th scope="col">{{ __('app.fields.bought_from') }}</th>
                        <th scope="col">{{ __('app.fields.bought_material') }}</th>
                        <th scope="col">{{ __('app.fields.code') }}</th>
                        <th scope="col">{{ __('app.fields.price') }}</th>
                        <th scope="col">{{ __('app.fields.bought_quantity') }}</th>
                ...
                <tbody>
                    @foreach ($boughtMaterials as $boughtMaterial)
                        <tr>
                            <td class="align-middle">{{ $boughtMaterial->bought_on }}</td>
                            <td class="align-middle">{{ $boughtMaterial->partner->name }}</td>
                            <td class="align-middle">{{ $boughtMaterial->material->name }}</td>
                            <td class="align-middle">{{ $boughtMaterial->material->code }}</td>
                            <td class="align-middle">{{ $boughtMaterial->price }}</td>
                            <td class="align-middle">{{ $boughtMaterial->quantity }}</td>
                            <td class="align-middle">{{ $boughtMaterial->invoice_num }}</td>
