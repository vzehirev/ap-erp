...
4. **A read-only database.** Every connection the web process opens runs
   ...
   root and unwritable while every process that executes application code runs
   as `demo`. Verified: reads and the reports' aggregate SQL are unaffected;
   INSERT, UPDATE, DELETE and CREATE TABLE all fail with *attempt to write a
   readonly database*.
   ...
   nginx and php-fpm keep their **master** process as root and run their
   **workers** as `demo`, which is how the official images for both are built.
   The fully-unprivileged arrangement this started as does not survive contact
   with Docker: the container's stderr is a root-owned pipe, a non-root process
   cannot reopen it, and nginx refuses to start when it cannot open its error
   log. Nothing that handles a request runs as root.
